...
(function (window) {
  ...
  for (i = 0; i < FREQUENCY_LEN; i++) {
    s_frequencyMap[i] =
      440.0 *
      Math.pow(2.0, (i - 69 * PITCH_RESOLUTION) / (12.0 * PITCH_RESOLUTION));
  }
  ...
  PicoWaveOscilator.prototype.next = function () {
    ...
    if (t.subform === 5) {
      t.noise =
        (2.0 * S_PI) / (1000 + Math.pow((10000 - f) / 10000, 10) * 2000000);
    }
    ...
    switch (t.subform) {
      ...
      case 4:
        s +=
          v *
          0.5 *
          ((w > S_PI ? 2 * S_PI - w : w) /*-S_PI/2*/ / (S_PI / 2)) *
          4; //三角波(ファミコン的)
        break;
      case 5:
        ...
        s +=
          v *
          1.0 *
          (noise_lut[Math.floor((w * NOISE_LONG) / (2 * S_PI))] / 128.0 - 1); //ノイズ
          ...
          default:
            if (
              (letterCode >= "a".charCodeAt(0) &&
                letterCode <= "g".charCodeAt(0)) ||
              letterCode === "r".charCodeAt(0) ||
              letterCode === "p".charCodeAt(0)
            ) {
              let note = 0;
              let nlen = 0;
              let noteFlag = 1;
              ...
              //シャープ//
              if (noteFlag) {
                if (t.mml.charAt(t.p) === "#" || t.mml.charAt(t.p) === "+") {
                  nextPtr(1);
                  note++;
                }
              }
              ...
              {
                let zenonpu = 384 * t.m_spt;
                t.noteoff =
                  t.notect -
                  ((t.notect * t.m_gate) / t.m_maxGate -
                    (t.m_gate2 * zenonpu) / 192);
              }
              //ポルタメント//
              if (t.mml.charAt(t.p) === "*") {
                nextPtr(1);
  ...
  const PicoPicoGenerator = function () {
  ...
  */
  ...
  picopicosound.play = function (mml, track) {
    if (!this.initialize) {
      this.start();
    }
    if (this.initialize) {
      this.generator[track].track.play(mml, true);
    }
  };

  picopicosound.stop = function (track) {
    if (!this.initialize) {
      this.start();
    }
    if (this.initialize) {
      this.generator[track].track.stop();
    }
  };

  window.SOUND = picopicosound;
})(window);
